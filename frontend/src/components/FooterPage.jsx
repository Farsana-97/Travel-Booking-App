import React from "react";

const FooterPage = () => {
  return (
    <footer class="w-full bg-black p-10">
      <div class="flex flex-row flex-wrap items-center justify-center gap-y-6 gap-x-12 bg-black text-center md:justify-between">
        <svg
          fill="#ffffff"
          version="1.1"
          id="Capa_1"
          xmlns="http://www.w3.org/2000/svg"
          xmlns:xlink="http://www.w3.org/1999/xlink"
          width="64px"
          height="64px"
          viewBox="-4.33 -4.33 37.55 37.55"
          xml:space="preserve"
          stroke="#000000"
          stroke-width="0.00028886"
        >
          <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
          <g
            id="SVGRepo_tracerCarrier"
            stroke-linecap="round"
            stroke-linejoin="round"
            stroke="#CCCCCC"
            stroke-width="0.751036"
          ></g>
          <g id="SVGRepo_iconCarrier">
            {" "}
            <g>
              {" "}
              <path d="M24.91,11.595c-0.385-3.055-2.963-5.426-6.117-5.426c-1.51,0-2.873,0.559-3.946,1.453c-1.011-1.291-2.565-2.137-4.33-2.137 c-2.636,0-4.832,1.85-5.38,4.32C2.271,10.006,0,12.369,0,15.285c0,3.045,2.471,5.517,5.519,5.517c0.647,0,1.262-0.131,1.839-0.338 c1.005,1.381,2.62,2.287,4.456,2.287c0.675,0,1.314-0.137,1.911-0.359c0-0.432,0-3.252,0-6.122l-6.772,3.067v-0.97l1.749-1.481 v-1.428h0.763v0.779l1.622-1.373v-1.346h0.763v0.699l1.876-1.59c0-1.361,0-2.42,0-2.767c0-1.15,0.711-2.213,0.717-2.206 C14.449,7.647,15.16,8.71,15.16,9.86c0,0.347,0,1.405,0,2.767l1.876,1.59v-0.699h0.762v1.346l1.623,1.373v-0.779h0.762v1.428 l1.75,1.481v0.97l-6.772-3.067c0,2.474,0,4.907,0,5.812c0.959,0.812,2.18,1.32,3.534,1.32c2.203,0,4.09-1.303,4.975-3.17 c0.27,0.053,0.549,0.084,0.834,0.084c2.42,0,4.383-1.961,4.383-4.379C28.882,13.656,27.132,11.806,24.91,11.595z"></path>{" "}
            </g>{" "}
          </g>
        </svg>
        <div>
          <h1 className="text-2xl font-bold text-teal-700 pl-55"
          >
            TravelVista
          </h1>
        </div>
        
        
        <ul class="flex flex-wrap items-center gap-y-2 gap-x-8">
          
          <li>
            <a
              href="/about-us"
              class="text-white hover:text-slate-500 focus:text-slate-500 text-sm"
            >
              About Us
            </a>
          </li>
          <li>
            <a
              href="/destination-view"
              class="text-white hover:text-slate-500 focus:text-slate-500 text-sm"
            >
              Destination
            </a>
          </li>
          <li>
            <a
              href="#"
              class="text-white hover:text-slate-500 focus:text-slate-500 text-sm"
            >
              Contribute
            </a>
          </li>
          <li>
            <a
              href="#"
              class="text-white hover:text-slate-500 focus:text-slate-500 text-sm"
            >
              Contact Us
            </a>
          </li>
        </ul>
      </div>
      <p class="block mb-4 text-sm text-center text-slate-500 md:mb-0 border-t border-slate-200 mt-4 pt-4">
        Copyright © 2025&nbsp;
        <a
          href="https://material-tailwind.com/"
          target="_blank"
          rel="noreferrer"
        >
          TravelVista
        </a>
        .
      </p>
    </footer>
  );
};

export default FooterPage;
